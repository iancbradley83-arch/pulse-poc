...
def format_help() -> str:
    return (
        ...
        "  /snooze [kind] [dur] suppress alerts: cost deploy health feed deeplink all\n"
        "\n"
        "  cost alerts now have [PAUSE] [BREAKDOWN] [DISMISS] buttons\n"
        "  /blacklist deferred — needs Pulse /admin/blacklist endpoint\n"
        "\n"
        "stage 4 lite\n"
        "  /preview             top 5 cards by relevance + deep_link HEAD status\n"
        "  /incident start|note|close  postmortem capture\n"
        "\n"
        "  /restore deferred — needs Pulse /admin/restore endpoint\n"
        "\n"
