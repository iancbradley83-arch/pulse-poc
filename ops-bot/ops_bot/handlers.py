...
Stage 2 commands: /feed, /feed page <n>, /card <id>, /embed <slug>,
                  /logs [n], /runbook <topic>, /env <key>
...
async def cmd_card(message: Message) -> None:
    ...
    # Match by full id first, then by 8-char prefix.
    matched = None
    for card in cards:
        cid = card.get("id") or ""
        if cid == card_id:
